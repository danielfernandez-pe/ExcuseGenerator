...
final class ChooseMediumCoordinator: BaseCoordinator<RouterResult<Void>> {
    ...
    override func start() -> AnyPublisher<CoordinationResult, Never> {
        let viewData = ChooseMediumView.ViewData()
        let view = ChooseMediumView(viewData: viewData)
        let viewController = ChooseMediumViewController(viewData: viewData, rootView: view)
